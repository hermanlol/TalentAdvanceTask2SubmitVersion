    ...
    public class ProfileService : IProfileService
    {
        ...
        public async Task<TalentProfileViewModel> GetTalentProfile(string Id)
        {
            ...
            if (profile != null)
            {
                ...
                profilePhotoUrl = string.IsNullOrWhiteSpace(profile.ProfilePhoto)
                          ? ""
        ...
        public async Task<bool> UpdateTalentProfile(TalentProfileViewModel model, string updaterId)
        {
            try
            {
                if (model.Id != null)
                {
                    //Find a model that fit for TalentProfile
                    ...
                    //Adding or Editing "skills" to User's profile
                    ...
                    //Adding or Editing "languages" to User's profile
                    ...
                    foreach (var item in model.Languages)
                    {
                        ...
                        UpdateLanguagesFromView(item, language);
                        newLanguages.Add(language);
                    }

                    //Adding or Editing "experiences" to User's profile
                    ...
                    //Place values into variables with skills, languages and experiences on userprofile
        ...
        {
            var users = (await _userRepository.Get(x => !x.IsDeleted)).Skip(position).Take(increment);
            var users2 = _userRepository.GetQueryable().Skip(position).Take(increment);
        ...
        protected void UpdateLanguagesFromView(AddLanguageViewModel model, UserLanguage original)
        {
